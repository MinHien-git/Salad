    ...
    public abstract class SHorizontalOrVerticalLayoutGroup : SLayoutGroup
    {
        ...
        protected void SetChildrenAlongAxis(int axis, bool isVertical)
        {
            ...
            if (alongOtherAxis)
            {
                ...
                for (
                    int i = startIndex;
                    m_ReverseArrangement ? i >= endIndex : i < endIndex;
                    i += increment
                )
                {
                    RectTransform child = rectChildren[i];
                    ...
                    float requiredSpace = Mathf.Clamp(
                        innerSize,
                        min,
                        flexible > 0 ? size : preferred
                    );
                    ...
                    {
                        float offsetInCell =
                            (requiredSpace - child.sizeDelta[axis]) * alignmentOnAxis;
                        SetChildAlongAxisWithScale(
                            child,
                            axis,
                            startOffset + offsetInCell,
                            scaleFactor
                        );
                    }
                }
